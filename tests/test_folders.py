...
class FolderResolutionTests(unittest.IsolatedAsyncioTestCase):
    async def test_returns_chats_for_matching_folder_case_insensitive(self) -> None:
        chat_a = SimpleNamespace(id=1, title="Sample Chat A")
        chat_b = SimpleNamespace(id=2, title="Sample Chat B")
        target_filter = FakeDialogFilter(title="Sample Folder", include_peers=["peer_a", "peer_b"])
        other_filter = FakeDialogFilter(title="Other Folder", include_peers=["peer_c"])
        client = FakeClient(
            FakeFiltersResult([other_filter, target_filter]),
            {"peer_a": chat_a, "peer_b": chat_b},
        )

        chats = await get_folder_chats(client, "sample folder")

        self.assertEqual(chats, [chat_a, chat_b])

    async def test_handles_rich_text_title_object(self) -> None:
        rich_title = SimpleNamespace(text="Sample Folder")
